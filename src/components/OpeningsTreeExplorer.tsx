...
import OpeningTreeNodeCard from "./OpeningTreeNodeCard";
...
export type OpeningsTreeActionTarget = {
  targetFen: string;
  line: string[];
...
export type OpeningsTreeExplorerProps = {
  route: OpeningsTreeRoute;
  onSelectLine: (line: string[]) => void;
  onCanonicalLine?: (line: string[]) => void;
  expandedAction?: {
    label: string;
    onSelect: (target: OpeningsTreeActionTarget) => void;
  };
};
...
function TreeColumnView({
  ...
  expandedAction?: OpeningsTreeExplorerProps["expandedAction"];
}) {
        ...
        {column.nodes.map((node) => {
          const card = node.isExpanded ? (
            <OpeningTreeNodeCard
              variant="expanded"
              node={node.view}
              // The synthesized root has no target FEN, so it never receives an
              // expanded action. Callers choose the label and behavior.
              footerAction={
                node.childFen != null && expandedAction ? (
                  <button
                    type="button"
                    className="tree-node-card__action-button"
                    onClick={() =>
                      expandedAction.onSelect({
...
/**
 * Reusable chesstree-style horizontal move tree synced with a board. The route
 * is controlled by the caller; this component owns request state, board input,
 * connectors, keyboard backtracking, and the five loading/error states.
 */
function OpeningsTreeExplorer({
  route,
  onSelectLine,
  onCanonicalLine,
  expandedAction,
}: OpeningsTreeExplorerProps) {
  const { playerColor, moves, opening } = route;
  const instanceId = useId().replace(/:/g, "");
  const boardId = `openings-tree-board-${instanceId}`;
  const arrowMarkerId = `openings-tree-arrowhead-${instanceId}`;
  const selectedArrowMarkerId = `${arrowMarkerId}-selected`;
  ...
  // only drive the click path and the dots overlay. The position + perspective
  // key invalidates stale hints by derivation, without a state-reset effect.
  const boardContextKey = `${playerColor}\u0000${view?.board.fen ?? ""}`;
  const [moveHints, setMoveHints] = useState<{
    contextKey: string;
    selectedSquare: string | null;
    optionSquares: Record<string, React.CSSProperties>;
  }>({
    contextKey: boardContextKey,
    selectedSquare: null,
    optionSquares: {},
  });
  const hintsMatchBoard = moveHints.contextKey === boardContextKey;
  const selectedSquare = hintsMatchBoard ? moveHints.selectedSquare : null;
  const optionSquares = hintsMatchBoard ? moveHints.optionSquares : {};
  ...
  const selectionLine = useMemo(() => view?.selectionLine ?? [], [view]);
  ...
  // Report canonicalization only when the rendered view is settled for the
  // current route, so a stale response kept on screen during a refetch can
  // never rewrite a freshly-selected line backward.
  useEffect(() => {
    if (!isSettled || !canonicalLine || !onCanonicalLine) {
      return;
    }
    onCanonicalLine(canonicalLine);
  }, [isSettled, canonicalLine, onCanonicalLine]);

  // Every navigation path flows through the controlled callback: cards,
  // headers, board input, and keyboard backtracking.
  const selectLine = useCallback(
    (newLine: string[]) => onSelectLine(newLine),
    [onSelectLine],
  );
  ...
  return (
    <div className="openings-tree-explorer">
...
export default OpeningsTreeExplorer;

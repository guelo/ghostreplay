 ...
 * (App.css <=720 / <=767 / <=760h / <=680h / <=1099 / max-height:640), not
 * generic device buckets. Pages with no responsive variance in a band reuse a
 * single capture via the per-page subsets exposed by {@link viewportsFor}.
 */
...
export const VIEWPORTS: Record<string, Viewport> = {
  ...
  mixed: { name: "mixed", width: 740, height: 900 },
 ...
 * bands. Game/analysis pages span the full set because the mixed (740) and
...
export const captureState = async (
  ...
  const buffer = await pw.screenshot({ path: absPath, fullPage });

...
const getToken = async (page: Page): Promise<string> => {
  const token = await page.evaluate(() => localStorage.getItem('ghost_replay_token'))
  if (!token) {
...
const startNewGameAsWhite = async (page: Page): Promise<void> => {
  await page.locator('.game-end-banner').getByRole('button', { name: /new game/i }).click()
  ...
  await expect.poll(async () => {
    return page
      ...
      .first()
      .textContent()
  }, { timeout: 15_000 }).toContain('Active')
}

const resignCurrentGame = async (page: Page): Promise<void> => {
  await page.getByRole('button', { name: 'Resign' }).click()
  await expect(page.locator('.game-end-banner-message')).toContainText('You resigned.')
}
...
test('seeded due blunder flow: game -> ghost review fail -> ghost review pass -> SRS updates', async ({
  ...
  await expect(page.getByRole('heading', { name: 'Blunder Library' })).toBeVisible()
  await expect(page.locator('.blunder-card').filter({ hasText: 'Nxe5' }).first()).toBeVisible()
  ...
  await expect(page.locator('.ghost-mode-label')).toHaveText('Ghost')
  ...
  await page.goto('/blunders')
  await expect(page.getByRole('heading', { name: 'Blunder Library' })).toBeVisible()
  await page.locator('.blunder-card').filter({ hasText: 'Nxe5' }).first().click()
  await expect(
    page

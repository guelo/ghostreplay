...
**Terminal row reconcile (`g-short-move-rows`).** The terminal PGN and the move rows travel in separate transactions, so a final upload that never commits would end a session describing more plies than it stores (47 historical sessions, 2026-04-06 … 2026-07-21). `end_game` therefore calls `app/terminal_row_reconcile.reconcile_terminal_move_rows` after assigning the terminal PGN and before the accuracy recompute: when the stored rows are a **verified prefix** of the PGN mainline (coordinates, move identity via SAN parse, and the normalized FEN chain all agree), the missing tail rows are derived from the PGN inside the terminal transaction — SAN/FEN only, evals NULL, segment via `segment_for_move` — so the same commit that flips `status='ended'` persists the full canonical grid, and strict-null accuracy refuses on the eval gap rather than the short row set. Fail-closed on an unparseable PGN, a diverging prefix (the `g-discard-branch-rows` shape), surplus rows (the `g-i6st` truncated-PGN non-defect), or a PGN over the derivation ceilings (`MAX_TERMINAL_PGN_BYTES` 32 KiB of strict UTF-8 — an O(1) code-point pre-gate bounds the string, then a strict encode of the remainder catches multi-byte inflation and refuses unpaired surrogates outright — / `MAX_DERIVABLE_PLIES` 600; `GameEndRequest.pgn` is unbounded and must not expand into unbounded INSERTs under the terminal lock). The parse is single and the refusal total: the reconcile's one `replay_pgn_mainline` call yields both the expected count and the derivation records (its reject semantics mirror the frozen accuracy parser, refusing only ever more), and `end_game` reuses `ReconcileResult.expected_plies` for both the accuracy recompute (`recompute_session_accuracy(expected_total_moves=…)`) and the analytics `ply_count` — so the terminal path parses the PGN exactly once, and a size-refused PGN exactly zero times (parser-spy pinned both ways). No receipt is written (receipts stay `final_full`-only), and a late `/moves` upsert overwrites derived rows with the client's richer record via ON CONFLICT. Recurrence is durably measurable through `game_sessions.derived_tail_rows`, stamped in the terminal transaction whenever derivation fires (it records that the reconcile fired, not current-row provenance — the late-upsert overwrite does not clear it); the post-commit `game_ended` event carries the same verdict (`row_reconcile_outcome`, `stored_move_rows`, `derived_tail_rows`) but is fire-and-forget, best-effort only. The historical cohort is repaired by the same decision path via `app/short_move_row_backfill.py` + `scripts/repair_short_move_rows.py` (dry-run-first; aggregate-only output enforced by disabling logging around plan/apply — `chess.pgn` logs SAN/FEN/headers for malformed movetext and serving-path helpers log session UUIDs — and by reporting failures as exception type only), stamping the same durable marker; sessions whose stored rows contradict their own PGN stay unrepaired strict-NULL residue.

...
- **Terminal deltas publish before the whole batch (g-delta-commit-decouple).** A terminal `SCORE_DELTA` enqueue carries the authoritative session ID into the existing serialized opening-score scheduler. Before the full recompute, coalesced sessions for one `(user, color)` share one overlay and one union-of-played-roots calculation with branch summaries disabled; each result is published separately in a bounded process-local cache under a monotonic per-session request generation. A publication binds the reloaded session identity/state/played chain, the full registry fingerprint, and a lower-bound `(evidence_seq, cache_epoch)` sampled before the overlay read. It also stores and hashes the exact raw/normalized fallback FEN scope, enforcing that the `AnalysisCache` row IDs hashed are exactly those passed to the viewer-association lookup. Sequence or registry drift rejects the publication. Epoch drift re-hashes only that stored shared scope: equal content CAS-rearms the publication to the sampled epoch, while an in-scope mutation rejects it. The poll reader reloads the session, prefers a provably-fresh persisted batch, otherwise accepts a valid scoped publication even when no batch exists, and finally serves a warm batch as unverified fallback. Scheduler pending/in-flight state is observability, not freshness, so the scoped result is readable while the same worker continues through full position/edge persistence. The cache and generations intentionally share the scheduler's **one-worker, one-replica** deployment assumption; adding either requires a shared publication/generation store. Restart or eviction is a safe miss.
...
- **Immediate first attempt.** The poll's sleep is **trailing**: attempt 0 fires with no delay, removing the guaranteed blind window. Retries keep the ~1500ms cadence (≈ the scheduler's quiet window), bounded by 28 attempts and a per-request `AbortSignal.timeout`. Phase 2's scoped result no longer waits for the whole-row commit, but it retains the quiet window and can queue behind a process-cold full job; the restored PostgreSQL 18 upper-bound measurement was 37.2s, so the budget is `1 + ceil((37.2s + 2 × 1.5s) / 1.5s) = 28` (about a 40.5s sampling span, not a strict wall-clock ceiling when requests time out).

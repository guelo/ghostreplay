...
def _create_test_schema(conn) -> None:
    ...
    conn.execute(text("""
        CREATE TABLE IF NOT EXISTS game_sessions (
            ...
            terminal_line_reconciled BOOLEAN NOT NULL DEFAULT 0,
            opening_phase_protocol_version SMALLINT,
            opening_phase_probe_ply INTEGER,
            opening_phase_probe_verdict VARCHAR(40),
            opening_middle_candidate_ply INTEGER,
            opening_middle_ready_at TIMESTAMP,
            opening_middle_ply INTEGER,
            opening_phase_exhausted BOOLEAN NOT NULL DEFAULT 0,
            opening_boundary_shadow_terminal_at TIMESTAMP,
            ...
            CONSTRAINT ck_game_sessions_move_line_revision CHECK (move_line_revision >= 0),
            CONSTRAINT ck_game_sessions_opening_phase_protocol CHECK (opening_phase_protocol_version IS NULL OR opening_phase_protocol_version = 1),
            CONSTRAINT ck_game_sessions_opening_probe_ply CHECK (opening_phase_probe_ply IS NULL OR opening_phase_probe_ply > 0),
            CONSTRAINT ck_game_sessions_opening_candidate_ply CHECK (opening_middle_candidate_ply IS NULL OR opening_middle_candidate_ply > 0),
            CONSTRAINT ck_game_sessions_opening_middle_ply CHECK (opening_middle_ply IS NULL OR opening_middle_ply > 0),
            CONSTRAINT ck_game_sessions_opening_probe_verdict CHECK (opening_phase_probe_verdict IS NULL OR opening_phase_probe_verdict IN ('passed','wrong_row_count','coordinate_mismatch','nonstandard_start','illegal_or_discontinuous_line','exhausted','capped')),
            CONSTRAINT ck_game_sessions_opening_ready_requires_candidate_baseline CHECK (opening_middle_ready_at IS NULL OR (opening_middle_candidate_ply IS NOT NULL AND opening_score_baseline IS NOT NULL)),
            CONSTRAINT ck_game_sessions_opening_marker_requires_baseline CHECK (opening_middle_ply IS NULL OR (opening_score_baseline IS NOT NULL AND opening_middle_ply = opening_middle_candidate_ply)),
            CONSTRAINT ck_game_sessions_opening_exhausted_clears_state CHECK (NOT opening_phase_exhausted OR (opening_phase_probe_ply IS NULL AND opening_middle_candidate_ply IS NULL AND opening_middle_ready_at IS NULL AND opening_middle_ply IS NULL AND opening_phase_probe_verdict = 'exhausted')),
            CONSTRAINT ck_game_sessions_opening_shadow_requires_protocol CHECK (opening_boundary_shadow_terminal_at IS NULL OR opening_phase_protocol_version = 1),

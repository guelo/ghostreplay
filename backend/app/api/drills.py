...
def continue_drill(
    ...
    if session.drill_state not in {"active", "failed"}:
        raise HTTPException(status_code=400, detail="Drill cannot be continued from its current state")
...
def abandon_drill(
    ...
    if session.drill_state not in {"active", "failed", "abandoned"}:
